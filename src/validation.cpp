...
#if defined(NDEBUG)
# error "Genteshare Core cannot be compiled without assertions."
#endif
...
{
    if (nPrevHeight == 0) {
        return 284250 * COIN; //permine 1.35%
    }
    ...
    // each six month halvings start at 100 coin.
    nSubsidy >>= halvings;
    return nSubsidy;
    // double dDiff;
...
{
    return blockValue/2; // mrzeta modify 50% for masternode
...
void ThreadScriptCheck() {
    RenameThread("genteshare-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // GENTESHARE : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(GENTESHARE): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(GENTESHARE): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // GENTESHARE : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(GENTESHARE): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(GENTESHARE): spork is off, skipping transaction locking checks\n");
    }
