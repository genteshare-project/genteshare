...
const char * const BITCOIN_CONF_FILENAME = "genteshare.conf";
const char * const BITCOIN_PID_FILENAME = "genteshared.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "genteshare" is a composite category enabling all Genteshare-related debug output
            if(ptrCategory->count(string("genteshare"))) {
...
#else
    const char* pszModule = "genteshare";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GenteshareCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GenteshareCore
    // Mac: ~/Library/Application Support/GenteshareCore
    // Unix: ~/.gentesharecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GenteshareCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GenteshareCore";
#else
    // Unix
    return pathRet / ".gentesharecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty genteshare.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override genteshare.conf
